        ...
        if (isStaff && !isAdmin) {
             ...
             if (staff != null) {
                 ...
                 if (currentSession != null && currentTerm != null) {
                     ...
                     val assignedSubjectIds = subjectTeacherAssignments
                        .filter { it.schoolClass.id == classId }
                        .map { it.subject.id }
                        .toSet()
                        
                     if (assignedSubjectIds.isNotEmpty()) {
                         classSubjects = classSubjects.filter { assignedSubjectIds.contains(it.subject.id) }
                     } else {
                         // Fallback check: is this staff the class teacher for this class?
