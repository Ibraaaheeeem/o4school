...
interface StudentClassRepository : JpaRepository<StudentClass, UUID> {
    ...
    fun findBySchoolIdAndTermId(schoolId: UUID, termId: UUID): List<StudentClass>
    
    fun findBySchoolIdAndTermIdAndIsActive(schoolId: UUID, termId: UUID, isActive: Boolean): List<StudentClass>
    
    fun findBySchoolIdAndStudentIdInAndTermId(schoolId: UUID, studentIds: List<UUID>, termId: UUID): List<StudentClass>
    ...
    fun findBySchoolClassIdAndAcademicSessionIdAndTermIdAndIsActive(
        classId: UUID, academicSessionId: UUID, termId: UUID, isActive: Boolean, pageable: org.springframework.data.domain.Pageable
    ): org.springframework.data.domain.Page<StudentClass>
    
    fun countBySchoolClassIdAndIsActive(classId: UUID, isActive: Boolean): Long

    fun countBySchoolClassIdAndAcademicSessionIdAndTermIdAndIsActive(
        classId: UUID, academicSessionId: UUID, termId: UUID, isActive: Boolean
    ): Long
    ...
    @Query("SELECT sc FROM StudentClass sc JOIN FETCH sc.schoolClass c LEFT JOIN FETCH c.track tr LEFT JOIN FETCH c.department d LEFT JOIN FETCH d.track t WHERE sc.student.id = :studentId AND sc.isActive = true AND c.isActive = true")
    fun findByStudentIdWithClassAndTrack(@Param("studentId") studentId: UUID): List<StudentClass>

    @Query("SELECT sc FROM StudentClass sc JOIN FETCH sc.schoolClass c LEFT JOIN FETCH c.track tr LEFT JOIN FETCH c.department d LEFT JOIN FETCH d.track t WHERE sc.student.id = :studentId AND sc.academicSession.id = :sessionId AND sc.term.id = :termId AND sc.isActive = true AND c.isActive = true")
    fun findByStudentIdAndAcademicSessionIdAndTermIdWithClassAndTrack(
