...
open class WhatsAppService(
    ...
    open fun sendTextMessage(to: String, text: String, user: User? = null, schoolId: UUID? = null): Boolean {
        ...
        return try {
            ...
            logMessage(to, text + " (Error: $errorMsg)", MessageDirection.OUTGOING, "FAILED", null, user, schoolId)
            false
        } catch (e: Exception) {
    ...
    open fun getTemplates(): List<Map<String, Any>> {
        ...
        val headers = HttpHeaders()
        headers.setBearerAuth(properties.accessToken ?: run {
    ...
    open fun sendTemplateMessage(to: String, templateName: String, languageCode: String = "en_US", components: List<Map<String, Any>>, user: User? = null, schoolId: UUID? = null): Boolean {
        ...
        val headers = HttpHeaders()
        headers.contentType = MediaType.APPLICATION_JSON
        val token = properties.accessToken ?: return false
        ...
        return try {
            ...
            logMessage(to, "[Template: $templateName] $errorMsg", MessageDirection.OUTGOING, "FAILED", null, user, schoolId)
            false
        } catch (e: Exception) {
