...
class WhatsAppMessagingController(
    ...
    fun getTemplates(): List<Map<String, Any>> {
        val templates = whatsappService.getTemplates()
        println("Fetched ${templates.size} live templates from Meta")
        return templates
    }
    ...
    fun sendBroadcast(
        ...
        val templateParams = mutableListOf<Map<String, Any>>()
        if (!templateName.isNullOrBlank()) {
            // Collect template parameters: templateParam_1, templateParam_2, ...
            val paramMap = request.parameterMap
            val keys = paramMap.keys.filter { it.startsWith("templateParam_") }.sorted()
            keys.forEach { key ->
                val value = paramMap[key]?.firstOrNull() ?: ""
                templateParams.add(mapOf("type" to "text", "text" to value))
            }
        }

        val components = if (templateParams.isNotEmpty()) {
            listOf(mapOf("type" to "body", "parameters" to templateParams))
        } else emptyList()
        ...
        recipients.forEach { recipient ->
            ...
            if (recipient.phoneNumber != null) {
                if (!templateName.isNullOrBlank()) {
                    whatsappService.sendTemplateMessage(recipient.phoneNumber, templateName, components = components, user = user, schoolId = selectedSchoolId)
                } else if (!message.isNullOrBlank()) {
                    whatsappService.sendTextMessage(recipient.phoneNumber, message, user = user, schoolId = selectedSchoolId)
                }
            }
        }
